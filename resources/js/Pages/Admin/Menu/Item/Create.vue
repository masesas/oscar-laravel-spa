    ...
    <LayoutAuthenticated>
        ...
        <SectionMain>
            <SectionTitleLineWithButton :icon="mdiMenu" title="Add menu item" main>
                <BaseButton :route-name="route('menu.item.index', menu.id)" :icon="mdiArrowLeftBoldOutline" label="Back"
                    color="white" rounded-full small />
            </SectionTitleLineWithButton>
            <CardBox form @submit.prevent="form.post(route('menu.item.store', menu.id))">
                ...
                <FormField label="Link" :class="{ 'text-red-400': form.errors.uri }">
                    <FormControl v-model="form.uri" type="text" placeholder="Enter Link" :error="form.errors.name">
                        <div class="item-list">
                            You can also enter an internal path such as <em class="placeholder">/home</em> or an external
                            URL such as <em class="placeholder">http://example.com</em>.
                ...
                <FormField label="Parent Item" :class="{ 'text-red-400': form.errors.parent_id }">
                    <FormControl v-model="form.parent_id" type="select" placeholder="--ROOT--"
                        :error="form.errors.parent_id" :options="item_options">
                        <div class="text-red-400 text-sm" v-if="form.errors.parent_id">
                            {{ form.errors.parent_id }}
                        </div>
                        <div>
                            The maximum depth for a link and all its children is fixed. Some menu links may not be available
                            as parents if selecting them would exceed this limit.
                        </div>
                    </FormControl>
                </FormField>
                <FormField label="Weight" :class="{ 'text-red-400': form.errors.weight }">
                    <FormControl v-model="form.weight" type="text" placeholder="Enter Weight" :error="form.errors.weight">
